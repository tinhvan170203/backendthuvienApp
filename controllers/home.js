...
module.exports = {
  ...
  getActiceBook: async (req, res) => {
    try {
      const activeBook = await Books.find({show: true}).limit(20).populate('theloai');
      let theloai = await Loaisachs.find().sort({thutu: 1});
      res.status(200).json({ activeBook, theloai})
  ...
  getAllBookOfType: async(req, res) => {
    try {
      let id = req.params.id;
      let books = await Books.find({ theloai: id }).sort({createdAt: 1});
  ...
  getBookDetail: async(req, res)=>{
    try {
      let book = await Books.findById(req.params.id).populate("theloai");
      res.send(book)
    } catch (error) {
  ...
  getNewBooks: async(req, res) => {
    try {
      let books = await Books.find().sort({createdAt: -1}).limit(20);
      res.send(books)
    } catch (error) {
  ...
  getTheloai: async(req, res) => {
    try {
      const theloai = await Loaisachs.find().sort({thutu: 1})
      res.send(theloai)
    } catch (error) {
  ...
  searchBooksApp: async (req,res) =>{
    try {
      let books = await Books.find({
        tensach: { $regex: req.body.tensach, $options: "$ i" },
        tacgia: { $regex: req.body.tacgia, $options: "$ i" },
        // trichyeu: { $regex: trichyeu, $options: "$ i" },
        // nhaxuatban: { $regex: nhaxuatban, $options: "$ i" },
        theloaiString: { $regex: req.body.theloai, $options: "$ i" },
      })
        .populate("theloai")
        .sort({ thutu: 1 });
